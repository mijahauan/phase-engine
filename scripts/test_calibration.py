    ...
    def tune_all(
        ...
        for name, inst in self.instances.items():
            try:
                ...
                logger.info(f"  {name}: Tuned to {frequency_hz/1e6:.3f} MHz, "
                           f"SSRC={channel.ssrc}")
            except Exception as e:
        ...
        def capture_one(name: str, channel: ChannelInfo):
            ...
            while samples_captured < samples_needed:
                if time.time() - start_time > timeout:
                    logger.warning(f"{name}: Capture timeout, got {samples_captured}/{samples_needed}")
                    break
                time.sleep(0.1)
...
def test_sample_alignment(capture: MultiRadiodCapture, frequency_hz: float):
    ...
    for name, samps in samples.items():
        logger.info(f"  {name}: {len(samps)} samples, "
                   f"power={np.mean(np.abs(samps)**2):.2e}")
    ...
    logger.info(f"\nAlignment Result:")
    logger.info(f"  Delay: {result.delay_samples} samples "
               f"({result.delay_samples / 12000 * 1e6:.1f} µs)")
...
def main():
    ...
    parser.add_argument(
        "--radiod1", default="bee1-status.local",
        help="First radiod status address"
    )
    parser.add_argument(
        "--radiod2", default="bee4-status.local",
        help="Second radiod status address"
    )
    parser.add_argument(
        "--test-freq", type=float, default=None,
        help="Test frequency in MHz for sample alignment (default: auto-discover AM)"
    )
    parser.add_argument(
        "--qth-lat", type=float, default=39.0997,
        help="Receiver latitude (default: Kansas City)"
    )
    parser.add_argument(
        "--qth-lon", type=float, default=-94.5786,
        help="Receiver longitude (default: Kansas City)"
    )
    parser.add_argument(
