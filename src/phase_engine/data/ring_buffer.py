    ...
    def __init__(
        ...
        for name in stream_names:
            self._streams[name] = StreamState(
                name=name, buffer=np.zeros(self.buffer_size, dtype=dtype)
            )
        ...
        logger.info(
            f"AlignedRingBuffer: {len(stream_names)} streams, "
            ...
            for name, state in self._streams.items():
                ...
                if read_end <= self.buffer_size:
                    ...
                    result[name] = np.concatenate(
                        [state.buffer[read_start:], state.buffer[: read_end - self.buffer_size]]
                    )
