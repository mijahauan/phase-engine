...
class RadiodSource:
    ...
    def disconnect(self) -> None:
        ...
        for freq_hz, state in list(self._channels.items()):
            try:
                ...
                logger.warning(f"{self.name}: Failed to remove channel: {e}")
    ...
    def create_channel(
        ...
        logger.info(f"{self.name}: Created channel at {frequency_hz/1e6:.3f} MHz -> {state.multicast_address}:{state.port}")
