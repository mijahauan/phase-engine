...
class PatternPlotter:
    ...
    def simulate_covariance_matrix(self, 
                                 freq_hz: float, 
                                 targets: List[Tuple[float, float]], 
                                 nulls: List[Tuple[float, float]]) -> np.ndarray:
    ...
    def compute_weights(self, 
                       ...
                       null_coords: List[Tuple[float, float]]) -> np.ndarray:
    ...
    def generate_plot(self, 
        ...
        for t in targets:
            ...
            ax.plot(math.radians(bearing), 0, 'go', markersize=10, label=f'Focus: {t.get("name", "Target")}')
            
        # Plot null markers
        for n in nulls:
            bearing = calculate_bearing(self.qth_lat, self.qth_lon, n["lat"], n["lon"])
            ax.plot(math.radians(bearing), 0, 'ro', markersize=10, label=f'Null: {n.get("name", "Interferer")}')
